...
export function validateMarkdown(source) {
    const issues = [];
...
export function renderMarkdownHtml(source) {
    ...
    if (!raw)
        return '';
    try {
        const parsed = md.parse(raw, { async: false });
...
export function hasMarkdownErrors(issues) {
    return issues.some((item) => item.severity === 'error');
}
/** 列表卡片摘要：取首段 Markdown 源（与后端 import 逻辑近似） */
export function extractSummaryMarkdown(source) {
    const lines = source.split('\n');
    const block = [];
    let started = false;
...
/** @deprecated 仅用于纯文本场景 */
export function extractPlainSummary(source, maxLen = 280) {
...
export function buildMarkdownPreview(source) {
